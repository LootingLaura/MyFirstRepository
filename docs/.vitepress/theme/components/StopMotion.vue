...
// Use shared state across pages
const { currentIndex: index, isPlaying, direction } = useStopMotionState();
...
watch(index, (n) => {
  if (typeof window !== "undefined") {
    ...
    window.dispatchEvent(
      new CustomEvent("stopmotion:frame", { detail: { index: n, src } }),
    );
  }
});
...
onMounted(() => {
  if (typeof window !== "undefined") {
    const src = currentSrc.value || "";
    window.dispatchEvent(
      new CustomEvent("stopmotion:frame", {
        detail: { index: index.value, src },
      }),
    );
  }
});
